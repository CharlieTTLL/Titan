(function() {
	...
	function validateSession() {
		...
		var url = './login';
		var req = JSON.stringify({});	//将javascript形式转成JSON字符串
		...
		function(res) {
			var result = JSON.parse(res);	//把一个字符串转换成JSON格式

			if (result.status === 'OK') {
				onSessionValid(result);
			} else {
				onSessionInvalid(result);	//会执行这步
			}
		});
	}
	...
	function changeFavoriteItem(item_id) {
		...
		var favorite = li.dataset.favorite !== 'true'; // important way to
														// inverse original
														// favorite
